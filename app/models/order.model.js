...
Order.updateById = (id, order, result) => {
  sql.query(
    "UPDATE orders SET customer_id = ?, product_name = ?, quantity = ?, price = ?, total_amount = ?, order_date = ?, status = ? WHERE id = ?",
    [order.customer_id, order.product_name, order.quantity, order.price, order.total_amount, order.order_date, order.status, id],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
